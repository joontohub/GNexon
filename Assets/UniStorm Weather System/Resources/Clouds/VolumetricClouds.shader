Shader "UniStorm/Clouds/Volumetric"
{
    ...
    fixed4 frag(v2f i) : SV_Target
    {
        ...
        float3 ro = float3(_uWorldSpaceCameraPos.x, 0, _uWorldSpaceCameraPos.z);
		...
		if (_UseUniStormFog == 1)
		{
			...
			half3 c_sun = _SunColor * min(pow(max(0, dot(rd, normalize(_SunVector.xyz))), _SunAlpha) * _SunBeta, 1) * clamp(_SunControl, 0.0, 1.0);
			half3 c_moon = _MoonColor * min(pow(max(0, dot(rd, normalize(_MoonVector.xyz))), _SunAlpha) * _SunBeta, 1) * clamp(_MoonControl, 0.0, 1.0);
			...
			{
				half4 c;
				c.rgb = float4((AtmosphericFinal.rgb + (c_sun * _SunIntensity) + (c_moon * _MoonIntensity) + (getNoise(i.uv) * 1.5) - 0.0025) * sstep, (1.0 - clouds.a) * sstep);
				c.a = (1.0 - clouds.a) * sstep;
				final = c;
			}
			else
			{
				final = float4((AtmosphericFinal.rgb + c_sun * _SunIntensity + c_moon * _MoonIntensity) * sstep, (1.0 - clouds.a) * sstep);
			}
		}
		...
		if (_MaskMoon == 1)
		{
			final = float4(final.rgb, saturate(remap(final.a, 0.0, lerp(1.015, 0.94, saturate(rd.y)))));
		}
