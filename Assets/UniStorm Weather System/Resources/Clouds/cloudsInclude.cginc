...
#else
    m = smoothstep(0.0f, lerp(2.5f, _uCloudsBaseEdgeSoftness, fade), m + (lerp(_uCloudsCoverage + _uCloudsCoverageBias, _uCloudsCoverage + _uCloudsCoverageBias, fade) - 1.));
#endif
...
#else
			float3 detail = cloudMapDetail(p * 0.7, norY, 0.5);
...
#else
			light *= smoothstep(-0.03f, 0.075f, sundotup) - detail.r * 1.4;
#endif
