...
CGINCLUDE
	...
	half4 ComputeFog (v2f i, bool distance, bool height) : SV_Target
	{
		...
		half4 c_moon = half4(_MoonColor * min(pow(max(0, dot(v, normalize(_MoonVector.xyz))), _SunAlpha) * _SunBeta, 1) * clamp(_MoonControl, 0.0, 1.0), 0);

		//Apply dithering to colors to avoid banding.
		if (_EnableDithering == 1)
		{
			half4 c;
			c.rgb = half4(c_sky + (c_sun * _SunIntensity) + (c_moon * _MoonIntensity) + (getNoise(i.uv1) * 1.5), 0);
			c.rgb *= c_sky.a;
			c.a = c_sky.a;
			Final = c;
		}
		...
		return half4(lerp(Final, sceneColor, lerp(saturate(_FogBlendHeight*wsPos.y*0.007), 1, fogFac)), 0);
	}
